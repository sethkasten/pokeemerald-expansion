...
Format: newest entries at the top. Each entry notes the file(s) touched and the intent.

---

## NPCs & Scripts

### Sevii Islands — Celio / Ruby / Sapphire / Rainbow Pass sidequest repaired
...
- **Steven grants the Town-Map pages too** in [data/maps/MeteorFalls_StevensCave/scripts.inc](data/maps/MeteorFalls_StevensCave/scripts.inc): the fruit-salad hand-over now also `setflag FLAG_SYS_SEVII_MAP_123` alongside the `ITEM_TRI_PASS` grant, so the Town Map pages for Sevii 1-3 appear immediately.
...
- **Flag added** in [include/constants/flags.h](include/constants/flags.h): `FLAG_CAUGHT_ROAMING_EON_MON` at `SYSTEM_FLAGS + 0x23` (repurposed `FLAG_UNUSED_0x023`). Set the moment the player captures whichever Eon-duo member is roaming.
- **Roamer catch hook** in [src/battle_main.c](src/battle_main.c): the `BATTLE_TYPE_ROAMER` cleanup path around `SetRoamerInactive` now also `FlagSet(FLAG_CAUGHT_ROAMING_EON_MON)` when `gBattleOutcome == B_OUTCOME_CAUGHT`, giving scripts a scriptable signal that the roaming Lati was captured.
...
- [data/maps/PokemonLeague_LoreleisRoom_Frlg/scripts.inc](data/maps/PokemonLeague_LoreleisRoom_Frlg/scripts.inc)
- [data/maps/PokemonLeague_BrunosRoom_Frlg/scripts.inc](data/maps/PokemonLeague_BrunosRoom_Frlg/scripts.inc)
- [data/maps/PokemonLeague_AgathasRoom_Frlg/scripts.inc](data/maps/PokemonLeague_AgathasRoom_Frlg/scripts.inc)
- [data/maps/PokemonLeague_LancesRoom_Frlg/scripts.inc](data/maps/PokemonLeague_LancesRoom_Frlg/scripts.inc)
- [data/maps/PokemonLeague_ChampionsRoom_Frlg/scripts.inc](data/maps/PokemonLeague_ChampionsRoom_Frlg/scripts.inc)
- [data/maps/IndigoPlateau_PokemonCenter_1F_Frlg/scripts.inc](data/maps/IndigoPlateau_PokemonCenter_1F_Frlg/scripts.inc)

### Two Island / Indigo Plateau — FRLG Victory Road wired in
...
### Victory Road 1F / 2F / 3F — encounter tables merged for EMERALD build
Same pattern as Mt. Moon, Diglett's Cave, and Rock Tunnel — but FR and LG differ on each floor: FR has **Arbok** in slot 6, LG has **Sandslash**. To keep both LG-exclusive and FR-exclusive mons in a single unversioned table, the FR entry is renamed to unversioned and its duplicated final Marowak slot is swapped to Sandslash. The FR `_LeafGreen` entries are deleted.

Merged tables (all encounter_rate 7):
- `sVictoryRoad1F` — Machop 32, Geodude 32, Onix 40 / 43 / 46, Zubat 32, Arbok 44, Golbat 44, Marowak 44, Machoke 44 / 46, Sandslash 46.
- `sVictoryRoad2F` — Machop 34, Geodude 34, Primeape 42, Onix 45 / 48, Zubat 34, Arbok 46, Golbat 46, Marowak 46, Machoke 46 / 48, Sandslash 48.
- `sVictoryRoad3F` — same as 1F.

Files changed:
- [src/data/wild_encounters.json](src/data/wild_encounters.json) — replaced 6 FR/LG entries with 3 merged entries.
- [src/data/wild_encounters.h](src/data/wild_encounters.h) — regenerated (auto).

### Route 120 / Route 121 — Rock Tunnel entrances
...
### Rock Tunnel 1F / B1F — encounter tables merged for EMERALD build
Same pattern as Mt. Moon and Diglett's Cave: FR and LG tables are byte-identical for both floors, so the merge is a rename of the `_FireRed` entry (base_label loses its version suffix → auto-wrapped in `#ifdef EMERALD`) and deletion of the `_LeafGreen` duplicate.
...
Files changed:
- [src/data/wild_encounters.json](src/data/wild_encounters.json) — replaced 4 FR/LG entries with 2 merged entries.
- [src/data/wild_encounters.h](src/data/wild_encounters.h) — regenerated (auto).

### Route 112 / Route 111 — Diglett's Cave entrances
...
### Diglett's Cave B1F — encounter table merged for EMERALD build
Same story as Mt. Moon: `sDiglettsCaveB1F_FireRed` and `sDiglettsCaveB1F_LeafGreen` were byte-identical in the JSON, so the merge is a rename (`_FireRed` → base name, which auto-wraps in `#ifdef EMERALD`) plus deletion of the `_LeafGreen` duplicate.

Merged `sDiglettsCaveB1F` (encounter_rate 5): Diglett Lv 15–22 filling most slots, Dugtrio Lv 29 and Lv 31 in the two rare slots.

Files changed:
- [src/data/wild_encounters.json](src/data/wild_encounters.json) — replaced 2 FR/LG entries with 1 merged entry.
- [src/data/wild_encounters.h](src/data/wild_encounters.h) — regenerated (auto).
...
### Mt. Moon 1F / B1F / B2F — encounter tables merged for EMERALD build
Same problem as the FRLG Safari Zone: each Mt. Moon floor had two encounter tables (`_FireRed` + `_LeafGreen`) gated on `#ifdef FIRERED` / `#ifdef LEAFGREEN`, which meant zero encounters would fire in the Emerald build.

For Mt. Moon, the FR and LG land tables happen to be **identical species/level lists** in the JSON, so the merge reduces to a rename: the `_FireRed` entry keeps its data and its `base_label` loses the version suffix (which auto-wraps it in `#ifdef EMERALD` in the generator), and the `_LeafGreen` duplicate is deleted.
...
After defeating Steven in his cave (`FLAG_DEFEATED_METEOR_FALLS_STEVEN` already set by the existing battle), talking to him again now triggers a one-time trade: Steven is starving and wants "an enormous fruit salad" made from every berry the Lilycove post-Champion vendor sells. If the player has 1 of all 37 berries in their bag (`ITEM_CHERI_BERRY` through `ITEM_BELUE_BERRY`, plus `ITEM_CHILAN_BERRY` and `ITEM_ENIGMA_BERRY`), a YES/NO confirmation appears; on YES all 37 berries are removed and Steven hands over `ITEM_TRI_PASS`.
...
- All 37 present + YES → 37 `removeitem` calls, "feast" flavor msgbox, `giveitem ITEM_TRI_PASS`, `setflag FLAG_RECEIVED_STEVEN_TRI_PASS`, closing dialogue.
...
She opens a mart-style buy menu split across 5 price tiers (multichoice → dedicated Pokemart per tier):
...
**Engine extension: custom-priced Pokemarts.** Vanilla marts always use the item's global `GetItemPrice`, which means overriding a berry's price via `.price` would also inflate sell prices (berry-tree exploit). Instead, added a parallel per-mart price list.
...
The Safari Zone game normally has a single step budget (500 in Emerald, 600 in FRLG). Since we now let the player travel between the Emerald and Kanto safari maps mid-game via the warp pad, a single counter would let a walk in one region drain the other's budget.
...
### Safari Zone Northeast — Kanto Safari warp pad
...
**Rest House safety redirect** — Entering the Rest House door on FRLG Safari Zone Center previously exited the player back into the FRLG hub, trapping them in Kanto maps. All three Rest House exit warps now redirect to `MAP_ROUTE121_SAFARI_ZONE_ENTRANCE` (the Emerald Safari Zone entrance building), giving the player a controlled route back to Hoenn. A new interior warp landing tile was added to the entrance building at (14, 12), just above the Route 121 exit doormat.

Files changed for the redirect:
...
### FRLG Safari Zone encounter tables — merged for EMERALD build
Previously each of the four FRLG Safari Zone maps had two encounter tables — one gated by `#ifdef FIRERED` and one by `#ifdef LEAFGREEN`. In our Emerald build neither define is active, so the maps had **no wild encounters at all**. This blocked the whole point of routing the player there via the warp pad.

Each pair (`_FireRed` + `_LeafGreen`) is now merged into a single Emerald-gated table by giving the entry a `base_label` with no version substring (which the generator wraps in `#ifdef EMERALD`).

Merge rules per slot category (land / water / fishing):
- Interleave FR slot 0, LG slot 0, FR slot 1, LG slot 1, …
- Deduplicate by `(species, min_level, max_level)`, keeping first occurrence.
- Truncate to the fixed slot count (`land_mons` 12, `water_mons` 5, `fishing_mons` 10). Pad by repeating the last entry if short.
- `encounter_rate` = max of the two sources.
...
Files changed:
- [src/data/wild_encounters.json](src/data/wild_encounters.json) — replaced 8 FRLG safari entries with 4 merged entries (`sSafariZoneCenter`, `sSafariZoneEast`, `sSafariZoneNorthFrlg`, `sSafariZoneWest`).
- [src/data/wild_encounters.h](src/data/wild_encounters.h) — regenerated (auto).

### FRLG Safari Zone — Fuchsia City exit sealed
...
### Route 117 — Budew & Roselia added
- File: [src/data/wild_encounters.json](src/data/wild_encounters.json)
...
### Meditite / Medicham — Mt. Pyre & Victory Road
- File: [src/data/wild_encounters.json](src/data/wild_encounters.json)
...
- Levels for the added species use the highest level listed in their original table.
- Tables 2–9 are left in place (dead-code fallback if the Mystery Gift path is ever wired up).
...
- Added a two-way warp pair connecting Altering Cave (top section) to Cerulean Cave 1F, providing a direct Hoenn ↔ Kanto shortcut.
  - New Altering Cave warp id 1 at **(8, 2)** → `MAP_CERULEAN_CAVE_1F` warp id 8.
  - New Cerulean Cave 1F warp id 8 at **(33, 20)** (just north of the Cerulean City exit stairs) → `MAP_ALTERING_CAVE` warp id 1.
- Rationale: gives players a way to reach Kanto from Hoenn once they've cleared Route 103's water, and lets them return the same way. Places the Kanto-side arrival near the city-exit stairs so it's easy to leave into Cerulean City if desired.
- **Note:** The metatiles at these two coordinates must be set to a warp behavior (stair/hole/door) via Porymap for the warps to trigger on step. The `warp_events` JSON entries are in place; only the map blockdata still needs the corresponding tile behavior for both endpoints.

### Cerulean Cave — merged FireRed/LeafGreen encounters
- File: [src/data/wild_encounters.json](src/data/wild_encounters.json)
- The FRLG-ported Cerulean Cave tables had version-exclusive water and fishing slots (Psyduck line in FR, Slowpoke line in LG). Merged both variants so every player can catch species from both games.
- **1F water (both `sCeruleanCave1F_FireRed` and `sCeruleanCave1F_LeafGreen`)** → identical merged table: Psyduck 30–40, Slowpoke 30–40, Golduck 40–50, Slowbro 40–50, Golduck 45–55.
- **B1F water (both `sCeruleanCaveB1F_FireRed` and `sCeruleanCaveB1F_LeafGreen`)** → identical merged table: Psyduck 40–50, Slowpoke 40–50, Golduck 50–60, Slowbro 50–60, Golduck 55–65.
- **1F fishing (both variants)** → slot 9 changed to **Slowpoke Lv25–35** so both Psyduck (slot 8) and Slowpoke are catchable via Old/Good/Super Rod.
- **B1F fishing (both variants)** → slot 8 unified to **Psyduck Lv15–25**; slot 9 kept as Gyarados Lv25–35 (Slowpoke still available via B1F water).
...
### Desert Underpass — Kanto fossils available
- File: [src/data/wild_encounters.json](src/data/wild_encounters.json)
...
### Mt. Pyre Summit — Chingling available
- File: [src/data/wild_encounters.json](src/data/wild_encounters.json)
- Change: `MAP_MT_PYRE_SUMMIT` land slot originally holding a duplicate Chimecho (Lv28) → **Chingling (Lv24)**.
...
### Route 114 — Zangoose available
- File: [src/data/wild_encounters.json](src/data/wild_encounters.json)
- Change: Slot originally holding a duplicate Seviper (Lv17) → **Zangoose (Lv17)**.
...
### Meteor Falls (B1F 2R) — Lunatone available
- File: [src/data/wild_encounters.json](src/data/wild_encounters.json)
- Change: Slots originally holding duplicate Solrock → **Lunatone**.
